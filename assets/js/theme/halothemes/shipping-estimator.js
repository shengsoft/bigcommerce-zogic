...
export default class ShippingEstimator {
    constructor($scope, $element) {
        ...
        if(url == "/cart.php"){
            this.$scope.find('.cart-total').remove();
        }
    }
    ...
    bindEstimatorEvents() {
        const $estimatorContainer = $(this.$scope).find('.shipping-estimator');
        const $estimatorForm = $(this.$scope).find('.estimator-form');
        ...
        var url = location.pathname;
        $(this.$scope).on('click', '.shipping-estimate-show', (event) => {
            event.preventDefault();

            var url = location.pathname;
            if(url == "/cart.php"){
                $('[data-cart] .shipping-estimate-show').click();
                $('#cart-preview-dropdown .cart-total').remove();

            }
            $(event.currentTarget).hide();
            $estimatorContainer.removeClass('u-hiddenVisually');
            $('.shipping-estimate-hide', this.$scope).show();
        });


        $(this.$scope).on('click', '.shipping-estimate-hide', (event) => {
            event.preventDefault();

            if(url == "/cart.php"){
                $('[data-cart] .shipping-estimate-hide').click();
                $('#cart-preview-dropdown .cart-total').remove();
            }
            $estimatorContainer.addClass('u-hiddenVisually');
            $('.shipping-estimate-show', this.$scope).show();
            $('.shipping-estimate-hide', this.$scope).hide();
        });
    }
}
